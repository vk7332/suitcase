import { detectContradiction } from "./contradiction-service.ts";
import { scoreWeakness } from "./weakness-service.ts";
import { decideStrategy } from "./strategy-service.ts";
import { decideTiming } from "./timing-service.ts";
import { generateObjections } from "./objection-service.ts";
import { generateFollowUp } from "./followup-service.ts";
import { generateCrossExamChain } from "./cross-exam-service.ts";
import { calculateWinProbability } from "./win-probability-service.ts";
import { aiJudgeMood } from "./judge-mood-service.ts";
import { calculateConfidence } from "./confidence-service.ts";
import { applySafety } from "./safety-service.ts";
import { checkCooldown } from "./cooldown-service.ts";

export const runCoCounsel = async ({
    text,
    role,
    memory,
}: {
    text: string;
    role: "opponent" | "judge";
    memory: any;
}) => {
    // 🧠 ANALYSIS
    const contradiction = detectContradiction(
        text,
        memory.history,
        memory.facts
    );

    const weakness = scoreWeakness(text);

    const judgeMood =
        role === "judge" ? (await aiJudgeMood(text)).mood : null;

    // ⚖ STRATEGY
    const strategy = decideStrategy({
        text,
        contradiction,
        role,
    });

    // ⏱ TIMING
    const timing = decideTiming({
        text,
        strategy: strategy.action,
        role,
        lastInterruptAt: memory.lastInterruptAt,
    });

    // 🗣 ACTIONS
    let objection = null;
    let followUp = null;
    let crossExam = null;

    if (strategy.action === "OBJECT" && timing.decision === "INTERRUPT_NOW") {
        const objectionResult = await generateObjections([text]);
        objection = objectionResult.objections?.[0]?.objection || null;
    }

    if (strategy.action === "CROSS") {
        crossExam = await generateCrossExamChain(
            text,
            memory.facts
        );
    }

    if (objection) {
        followUp = await generateFollowUp(
            text,
            objection,
            memory.facts
        );
    }

    // 📊 WIN
    const win = calculateWinProbability({
        weakness,
        contradiction,
        strategy,
    });

    const confidence = calculateConfidence({
        contradiction,
        weakness,
        timing,
    });

    const cooldownOk = checkCooldown(memory.lastActionAt);

    const safety = applySafety({
        strategy,
        timing,
        confidence,
    });

    // 🚫 BLOCK if unsafe
    if (!cooldownOk || !safety.allowed) {
        return {
            blocked: true,
            reason: !cooldownOk
                ? "Cooldown active"
                : safety.reason,
            confidence,
        };
    }

    // ✅ ALLOW
    memory.lastActionAt = Date.now();

    return {
        contradiction,
        weakness,
        strategy,
        timing,
        objection,
        followUp,
        crossExam,
        win,
        confidence,
        blocked: false,
    };
};
